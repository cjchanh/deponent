...
(self-assessed)** when the evaluator and the kernel share an author or
organisation — including kernels that are not Deponent, not from CDS, and not
forks of this code. The bare mark **GAK-conformant** is reserved for a run by a
party independent of the kernel's author. Self-assessed conformance is not
third-party certification. The mark attaches to the test result, not to this
project. Deponent is the reference implementation of the standard, not the owner
of the adjective.

## You may

- Describe your software, factually, as **GAK-conformant (self-assessed)** when it
  actually passes the harness (`python3 -m deponent.badge verify --kernel <yours>`
  exits 0) and the run is author-side. This is vendor-neutral and open to anyone —
  it is a claim about the standard, not about Deponent. Do not drop
  "(self-assessed)" unless a party independent of the kernel's author ran the
  harness.
...
- Claim a kernel is **GAK-conformant** or **GAK-conformant (self-assessed)** when it
  does not pass the harness. The mark is earned by a reproducible result, not
  asserted; the `verify` CLI fails closed. Self-assessed conformance is not
...
license. A renamed fork may still earn and claim the **GAK-conformant
(self-assessed)** mark — that mark belongs to the standard, not to the Deponent
name.
