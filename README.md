...
**Seatbelt escape-proofs (red-team during development):** 0 of 9 escape attempts succeeded, including the two real Seatbelt bypasses — `osascript 'do shell script'` (the spawned shell is still sandboxed) and `launchctl submit` (no stray job) — plus loopback `/dev/tcp` egress, DNS, and symlink/hardlink/rename writes-out. All blocked. The network and write-out escapes, plus memory-bomb and wall-clock runaways, are promoted into live tests; the two documented Seatbelt bypasses are recorded as development red-team that shaped the gate denylist and the Seatbelt profile (`canaries/CANARIES.md`).
