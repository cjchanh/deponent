...
PROFILES (so a different governance SHAPE doesn't false-FAIL).
  ...
  live tool call: evaluate(tool, params) -> verdict). A commit-gate implementation
...
class ConformanceReceipt:
    ...
    def conformant(self) -> bool:
        ...
        return all(r.status != "FAIL" for r in self.results) and any(r.status == "PASS" for r in self.results)
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "clauses": [
                {"id": r.id, "profile": r.profile, "status": r.status, "detail": r.detail}
                for r in self.results
            ],
        }

    def render(self) -> str:
        lines = [f"GAK CONFORMANCE — {self.kernel}  (profile: {self.profile})", "=" * 60]
        for r in self.results:
...
def run_conformance(kernel: KernelAdapter, clauses: tuple[Clause, ...] = CLAUSES) -> ConformanceReceipt:
    ...
    for c in clauses:
        ...
        if c.requires and c.requires not in kernel.supports:
            results.append(
                ClauseResult(c.id, c.profile, "NA", f"kernel does not claim '{c.requires}'. {c.statement}")
            )
            continue
        try:
            ...
            results.append(
                ClauseResult(c.id, c.profile, "FAIL", f"check raised {type(e).__name__}: {e}. {c.statement}")
            )
            continue
