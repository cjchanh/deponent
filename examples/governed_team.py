...
WORK = Path(tempfile.mkdtemp(prefix="deponent-team-"))
...
def architect(goal: str, module_stem: str, backend) -> str:
    base = (
        "You are the ARCHITECT. Write a single pytest file named test_acceptance.py "
    ...
    attempts = [
        (3000, " Return ONLY the file content in one ```python fence."),
...
def builder_loop(cell: Cell, goal: str, module: str, backend, max_steps=12):
    ...
    for step in range(1, max_steps + 1):
        ...
        if not turn["tool_calls"]:
            ...
            chk = (
                cell.act("run_cmd", {"cmd": "python -m pytest -q test_acceptance.py"}, agent="builder")
                if mod_exists
                else None
            )
            ...
            why = (
                f"{module} has not been created yet"
                if not mod_exists
        ...
        for c in turn["tool_calls"]:
            ...
            print(
                f"  [step {step}] [{mark}] {c['name']}({argstr}) "
                f"[{r.decision.blast_class}] -> {r.output.replace(chr(10), ' ')[:90]}",
                flush=True,
            )
...
def reviewer(goal, code, test_src, pytest_out, backend) -> dict:
    base = (
        "You are the REVIEWER (independent, ADVISORY). Flag if the work does NOT genuinely "
...
def main():
    ...
    ap.add_argument(
        "--model", default=None, help="model id/path (env DEPONENT_MLX_MODEL for the mlx default)"
    )
    args = ap.parse_args()
    ...
    blocked = WORK / "blocked-example"
    blocked.mkdir(exist_ok=True)
    sentinel = blocked / "sentinel.txt"
    sentinel.write_text("keep", encoding="utf-8")
    command = "rm -rf ./blocked-example"
    r = cell.act("run_cmd", {"cmd": command}, agent="builder")
    preserved = sentinel.read_text(encoding="utf-8") == "keep"
    print(
        f"  injected: run_cmd {command!r} -> {r.decision.verdict} "
        f"[{r.decision.blast_class}]; sentinel_preserved={preserved}",
        flush=True,
    )
    ...
    vr = subprocess.run(
        "python -m pytest -q test_acceptance.py", shell=True, cwd=WORK, capture_output=True, text=True
    )
    ...
    print(
        f"  test_pass={test_pass}  chain_intact={ok}  gate_blocked_rogue={blocked >= 1}  reviewer(advisory)={rv}",
        flush=True,
    )
    print(
        f"  => {'GOVERNED PASS — built, gated, testified' + advisory_flag if overall else 'INCOMPLETE'}",
        flush=True,
    )
