#!/usr/bin/env python3
"""safety_action_governor.py — use deponent to testify safety decisions.
...
In a real integration the `_execute` hook would talk to the actual actuator bus;
here it simulates the effector and records the outcome hash.
"""
...
SAFETY_ACTION_DENY_SUBSTR = BUILD_DENY_SUBSTR + SAFETY_ACTION_DENY_EXTRA


class SafetyActionGate(Gate):
    ...
    def evaluate(self, tool: str, params: dict) -> GateDecision:
        if tool == "authorize_fire":
            ...
            if cmd.confidence < 0.95:
                return GateDecision(
                    "BLOCK", "confidence-below-threshold", f"confidence {cmd.confidence} < 0.95"
                )
            if not cmd.human_affirm:
                return GateDecision(
                    "BLOCK", "human-in-the-loop-required", "fire requires explicit human affirmation"
                )
            if not cmd.roe_authorization:
                return GateDecision("BLOCK", "roe-denied", "target not in current rules-of-engagement")
            return GateDecision("ALLOW", "fire-authorized", "human + ROE + confidence pass")
        return super().evaluate(tool, params)


class SafetyActionCell(Cell):
    ...
    def _execute(self, tool: str, params: dict) -> str:
        if tool == "authorize_fire":
            ...
            return f"effector: WEAPON_RELEASED (target={cmd.target_id}, confidence={cmd.confidence})"
...
def main() -> int:
    ...
    gate = SafetyActionGate(
        sandbox, deny=SAFETY_ACTION_DENY_SUBSTR, allow_heads=frozenset()
    )  # no shell commands allowed
    cell = SafetyActionCell(sandbox, gate=gate, ledger_path=sandbox + "/safety.jsonl", use_jail=False)
    ...
    for target, conf, human, roe in scenarios:
        ...
        result = cell.act("authorize_fire", cmd.__dict__, agent="safety-action")
