...
(sandbox / "blocked-example").mkdir()
(sandbox / "blocked-example" / "sentinel.txt").write_text("keep", encoding="utf-8")
cell = Cell(sandbox, use_jail=False)  # use_jail=True on macOS adds the Seatbelt jail
...
print(cell.act("run_cmd", {"cmd": "rm -rf ./blocked-example"}).output)  # destructive -> BLOCK
